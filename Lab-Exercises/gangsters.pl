...
writeClauses(MaxConsecutiveHours):-
    atMostOneTaskPerGangsterAtHour,
    noTasksOnUnavailableHours,
    noTwoDifferentTasksOnConsecutiveHours,
    allTasksExactlyNGangster,
    noGangsterWorksMoreThanMaxConsecutiveHours(MaxConsecutiveHours),
    true,!.
writeClauses(_):- told, nl, write('writeClauses failed!'), nl,nl, halt.

atMostOneTaskPerGangsterAtHour :- gangster(G), hour(H), findall(does(G,T,H), task(T), L), atMost(1,L), fail.
atMostOneTaskPerGangsterAtHour.     %generates clauses as intended

noTasksOnUnavailableHours :- gangster(G), hour(H), \+available(G,H), task(T), writeClause( [-does(G,T,H)]), fail.
noTasksOnUnavailableHours.

noTwoDifferentTasksOnConsecutiveHours :- gangster(G), hour(H), hour(H1), H1 is H+1, task(T), task(T1), T \= T1, writeClause([ -does(G,T,H), -does(G,T1,H1)]), fail.
noTwoDifferentTasksOnConsecutiveHours.  %seems to work as intended

allTasksExactlyNGangster :- needed(T,H,N), findall( does(G,T,H), gangster(G), L), exactly(N,L), fail.
allTasksExactlyNGangster.

noGangsterWorksMoreThanMaxConsecutiveHours(Max) :- gangster(G), task(T), hour(H), H1 is H+Max, H1 =< 72, findall( does(G,T,H2), (hour(H2), between(H,H1,H2)), L), atMost(Max,L), fail. 
noGangsterWorksMoreThanMaxConsecutiveHours(_).
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
costOfThisSolution(M,Cost):- hour(Count), Cost is 72-Count+1, hour(InitHour), EndHour is InitHour+Cost-1, EndHour =< 72, gangster(G), findall( does(G,_,H), (hour(H), between(InitHour,EndHour,H), member(does(G,_,H), M)), L), sort(L, L1), length(L1, Cost), !.
