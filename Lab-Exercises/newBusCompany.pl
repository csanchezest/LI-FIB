...
%despres d'un trajecte, el bus comença des d'on ha acabat
%no es poden fer mes de maxDistance kilometres en 2 dies consecutius

%%%%%%  1. SAT Variables:

satVariable( bdcc(B,D,C1,C2) ):- bus(B), day(D), trip(C1-C2).  % bus B on day D does C1-C2
% ... more variables may be needed
%necessitarem una variable per dir els dies que un bus fa trajectes per tal de mirar que en fa com a minim 1 i com a molt K; tambe estara relacionada amb bdcc amb una doble implicacio <=> amb expressOr(variable de bus fa trajecte X dia, literals bdcc on bus fa trajecte X dia)

%%%%%%  2. Clause generation:

writeClauses:-  
    exactlyOneTripPerBusAndDay,     %cada dia un bus fa un trajecte
    consecutiveTrips,               %cada viatge ha de començar des d'on ha acabat l'anterior
    noIncompatibleTrips,
    eachTripOnlyOncePerDay,         %cada viatge es fa maxim 1 cop per dia
    eachTripAtLeastKTimesInWeek,    %cada viatge es fa minim F vegades per setmana
    true,!.
writeClauses:- told, nl, write('writeClauses failed!'), nl,nl, halt.

%siguin B un bus i D un dia: de tots els possibles trajectes que pot realitzar un bus en aquell dia, nomes en pot fer exactament 1 (cada dia un bus fa un trajecte)
exactlyOneTripPerBusAndDay :- bus(B), day(D), findall( bdcc(B,D,C1,C2), trip(C1-C2), L), exactly(1,L), fail.
exactlyOneTripPerBusAndDay.

noIncompatibleTrips :- bus(B), day(D), consecutiveDays(D,D1), trip(C1-C2), trip(C3-C4), C2 \= C3, writeClause([-bdcc(B,D,C1,C2),-bdcc(B,D1,C3,C4)]), fail.
noIncompatibleTrips.

%consecutiveTrips :- bus(B), day(D1), numDays(N), D1<N, trip(C1-C2), findall(bdcc(B,D2,C2,C3), (consecutiveDays(D1,D2), trip(C2-C3), C2 \= C1, C3 \= C2, \+tooLongDist(C1-C2-C3)), L), L \= [], expressOr(bdcc(B,D1,C1,C2), L), fail.
consecutiveTrips :- bus(B), day(D1), numDays(N), D1<N, trip(C1-C2), findall(bdcc(B,D2,C2,C3), (consecutiveDays(D1,D2), trip(C2-C3), C1 \= C2, C2 \= C3, \+tooLongDist(C1-C2-C3)), L), L \= [], writeClause([-bdcc(B,D1,C1,C2)|L]), fail.
consecutiveTrips.

%sigui C1-C2 un trajecte i D un dia donat: indiquem que, com a molt, nomes es pot fer 1 cop aquell viatge C1-C2 en el dia D
eachTripOnlyOncePerDay :- trip(C1-C2), day(D), findall(bdcc(B,D,C1,C2), bus(B), L), atMost(1,L), fail.
eachTripOnlyOncePerDay.

%sigui C1-C2 un trajecte i F el nombre de vegades que s'ha de fer a la setmana: marquem que, com a minim, el trajecte s'ha de fer F cops (independentment dels busos i/o dies en que es faci)
eachTripAtLeastKTimesInWeek :- frequency(C1-C2,F), findall(bdcc(B,D,C1,C2), (bus(B), day(D)), L), atLeast(F,L), fail.
eachTripAtLeastKTimesInWeek.
