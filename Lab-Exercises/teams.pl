...
compatibleWorkerScore(W1,W2):- workerScore(W1,S1), workerScore(W2,S2), maxScore(N), S is S1+S2, between(0,N,S).
...
writeClauses:-  %% Complete this!  
    eachWorkerExactlyOneTeam,
    eachTeamAcceptableSize,
    noIncompatibleWorkers,
    true,!.
writeClauses:- told, nl, write('writeClauses failed!'), nl,nl, halt.

%sigui W un treballador: cerca tots els possibles equips en que podria encaixar el treballador W, i en tries EXACTAMENT un d'ells
eachWorkerExactlyOneTeam :- worker(W), findall( wt(W,T), team(T), L), exactly(1,L), fail.
eachWorkerExactlyOneTeam.

%sigui T un equip: cerca tots els possibles treballadors candidats a estar dins l'equip i, comprovant que no en tinguem mes (o menys) dels necessaris, triem un
eachTeamAcceptableSize :- team(T), findall( wt(W,T), worker(W), L), minSize(S1), maxSize(S2), atMost(S2,L), atLeast(S1,L), fail.
eachTeamAcceptableSize.

%sigui T un equip: cerquem totes les combinacions de treballadors W1-W2 (sense discriminar repeticions) tals que siguin incompatibles entre ells; marquem que, en un equip, com a maxim nomes un d'ells pot estar-hi
noIncompatibleWorkers :- team(T), worker(W1), worker(W2), W1 \= W2, \+compatibleWorkerScore(W1,W2), atMost(1,[wt(W1,T),wt(W2,T)]), fail.
noIncompatibleWorkers.
