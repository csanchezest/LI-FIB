...
satVariable( villageWatched(V)) :- village(V).  % means "village V is being watched from an existing tower"
satVariable( towerVillage(V)) :- village(V).    % means "village V has a tower"
...
writeClauses(MaxNumTowers):-
    ...
    exactlyOneTowerPerSignificantVillage,
    atMostMaxTotalTowers(MaxNumTowers),
    relateTowersWithVillages,
    makeVillageWatched,
    villageHasTower,
...
atMostOneTowerPerVillage.   %works as intended

exactlyOneTowerPerSignificantVillage :- significantVillage(V), findall( towerPos(I,J), posVillage(V,I,J), L), exactly(1,L), fail.
exactlyOneTowerPerSignificantVillage.   %works as intended

atMostMaxTotalTowers(MaxNumTowers) :- findall( towerVillage(V), village(V), L), atMost(MaxNumTowers,L), fail.
atMostMaxTotalTowers(_).    

relateTowersWithVillages :- village(V), findall( towerPos(I,J), (posWatchesVillage(V,I,J), posVillage(_,I,J)), L), atLeast(1,L), fail.
relateTowersWithVillages.

villageHasTower :- posVillage(V,I,J), writeClause([ -towerPos(I,J), towerVillage(V)]), fail.
villageHasTower.

makeVillageWatched :- posWatchesVillage(V,I,J), writeClause([ -towerPos(I,J), villageWatched(V)]), fail.
makeVillageWatched.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
costOfThisSolution(M,Cost):- findall(position(I,J), member(towerPos(I,J), M), L), sort(L,L1), length(L1,Cost), !.
