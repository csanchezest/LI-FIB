...
%%%%%%  2. Clause generation:
writeClauses:-
    carExactlyOnePos,       %tot cotxe es troba exactament en 1 posicio
    posExactlyOneCar,       %tota posicio es ocupada exactament per 1 cotxe 
...
carExactlyOnePos :- 
    car(C),
    findall( carPos(C,P), position(P), L),
    exactly(1,L),
    fail.
carExactlyOnePos.
...
posExactlyOneCar :- 
    position(P),
    findall( carPos(C,P), car(C), L),
    exactly(1,L),
    fail.
posExactlyOneCar.

relateCarPosAndPosOpt :- 
    car(C), position(P), carOption(C,O),
    writeClause([ -carPos(C,P), posOpt(P,O)]),
    fail.
relateCarPosAndPosOpt.

optionConstraints:-
    option(Opt,Max,WindowSize), position(P),
    windowOfSize(WindowSize, P, PEnd),
    findall( posOpt(P1,Opt), (position(P1), P1 >= P, P1 =< PEnd), L),
    atMost(Max, L),
    fail.
optionConstraints.
