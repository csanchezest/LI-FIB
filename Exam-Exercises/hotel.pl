...
satVariable( ug(G) ) :- gangster(G).             % means "gangster G is unused" (has no room assigned)
...
writeClauses(NumUnAssigned):-
    ...
    oneRoomExactlyOneGangster,
    noEnemyGangstersNearby,
    relateGRwithUG,
    relateSmallerGangsterWithBiggerGangster,
...
oneRoomExactlyOneGangster :- room(R), findall( gr(G,R), gangster(G), L), atMost(1,L), fail.
oneRoomExactlyOneGangster.

noEnemyGangstersNearby :- enemies(G1,G2), neighborRooms(R1,R2), writeClause([ -gr(G1,R1), -gr(G2,R2)]), writeClause([-gr(G1,R2), -gr(G2,R1)]), fail.
noEnemyGangstersNearby.

relateGRwithUG :- gangster(G), room(R), writeClause([ -gr(G,R), -ug(G)]), fail.
relateGRwithUG.

% dubtes
relateSmallerGangsterWithBiggerGangster :- gangster(G), findall( gr(G1,_), (gangster(G1), G>G1),L), L \= [], expressAnd(gr(G,_), L), fail.
relateSmallerGangsterWithBiggerGangster.
...
costOfThisSolution(M,Cost):- findall( -ug(G), (gangster(G), \+member(gr(G,_), M)), L), sort(L,L1), length(L1,Cost), !.
